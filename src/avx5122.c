             ...
             double       *bk)
{
  ...
  for(j = 0; j < q; ++j)
  {
    for(i = 0; i < row; ++i)
    {
      ...
      bk += NR;
      mt += inc;*/
      bk[i*NR+j*row*NR:NR] = mt[i*inc+j*NR:NR]; // parallel
    }
    //mt += (NR - row*inc);
  }
  bk += q*row*NR; // parallel
  mt += q*NR; // parallel
  if(r > 0)
  {
    for(i = 0; i < row; ++i)
                   ...
                   int           ncol)
{
  ...
  //register __m512d _C18, _C19, _C1A, _C1B;
             ...
             const int              lc)
{
  ...
  #pragma omp parallel num_threads(17) private(i,mc,_A)
  {
    ...
    for(i = 0; i < mq; ++i)
    {
      ...
      //#pragma omp parallel num_threads(4) private(j,nc,p) shared(i,mc,_A)
